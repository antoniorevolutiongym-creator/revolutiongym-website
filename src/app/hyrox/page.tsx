...
export default function HyroxPage() {
  return (
    <main id="hyrox-page" className="min-h-screen bg-revolution-black text-white">
      ...
      <section id="timetable" className="bg-white py-28 text-revolution-black sm:py-36" aria-labelledby="timetable-title"><div className="mx-auto max-w-7xl px-6 lg:px-8"><p className="text-sm font-black uppercase tracking-[0.45em] text-revolution-red">Weekly timetable</p><h2 id="timetable-title" className="mt-5 font-display text-5xl font-black uppercase leading-[0.9] tracking-[-0.05em] sm:text-7xl">Orario settimanale.</h2><div className="mt-12 grid gap-4 md:grid-cols-2">{[{ label: "Mattina", days: "Lunedì · Mercoledì · Venerdì", time: "07:00 – 08:00" }, { label: "Sera", days: "Lunedì · Mercoledì · Venerdì", time: "19:00 – 20:00" }].map((slot) => <div key={slot.label} className="rounded-[2rem] border border-black/10 bg-zinc-100 p-8"><p className="text-xs font-black uppercase tracking-[0.28em] text-revolution-red">{slot.label}</p><h3 className="mt-4 text-2xl font-black uppercase">{slot.days}</h3><p className="mt-3 text-2xl font-black text-revolution-black">{slot.time}</p><p className="mt-2 text-zinc-600">Coach: Antonio Costagliola</p></div>)}</div></div></section>
